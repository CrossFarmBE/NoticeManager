...
class NoticeManager extends PluginBase implements Listener{
    ...
    public function onEnable() : void{
        ...
        $cmd = new PluginCommand("알림", $this, $this);
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
        if(!$sender instanceof Player || $command->getName() !== "알림"){
            return true;
        }
    ...
    public function keyword(Player $player) : void{
    ...
    public function onChat(PlayerChatEvent $ev) : void{
        ...
        $ev->setRecipients(array_filter($ev->getRecipients(), function($recipient) use ($format) : bool{
            ...
            foreach($arr as $keyword){
                if(str_contains(strtolower($format), $keyword)){
                    $recipient->sendMessage("§f[키워드 알림]" . $format);
                    CrossUtils::playSound($recipient, "cross.ketalk");
                    return false;
                }
            }
            return true;
